...
int global_test=0;
float global_record[][] = new float[4][1000];
...
public void recordData()
{
   if((global_record_count > 0) && (global_record_count % 1000 == 0 ))
   {
     float rand;
     int len, max = 0;
     for(int b = 0; b < 4; b++)
     {
       ...
       while(i < 1000)
       {
         rand = (float)(Math.random()*(2 * max + 1) - max);
         len = (int)(Math.random()*(401) + 100);
         if(i + len > 1000)
         {
           len = 1000 - i;
         }
  ...
  if( global_record_count < 1000 )
  {
    if((global_hr != 0) && (global_rr != 0) && (global_spo2 != 0) && (global_temp != 0))
    {
      global_record[0][global_record_count] = global_hr;
      global_record[1][global_record_count] = global_rr;
      global_record[2][global_record_count] = global_spo2;
      global_record[3][global_record_count] = global_temp;
      global_record_count++;
    }
  }
  else
  {
    if(global_hr != 0)
    {
      global_hr = (int)global_record[0][global_record_count%1000];
      if(global_hr < 0)
      {
        global_hr = 0; 
      }
    ...
    if(global_rr != 0)
    {
      global_rr = (int)(global_record[1][global_record_count%1000]);
      if(global_rr < 0)
      {
        global_rr = 0; 
      }
    ...
    if(global_spo2 != 0)
    {
      global_spo2 = (int)(global_record[2][global_record_count%1000]);
      if(global_spo2 < 0)
      {
        global_spo2 = 0;
      }
      else if(global_spo2 > 100)
      {
        global_spo2 = 100;
      }
    }
    if( global_temp != 0)
    {
      global_temp = (float)(global_record[3][global_record_count%1000]);
    }
    global_record_count++;
  }
}
  ...
  case CESState_PktLen_Found:
    ...
    } else  //All  and data received
    {
      ...
      {     
        ...
        global_temp = (float) ((int) CES_Pkt_Data_Counter[12]| CES_Pkt_Data_Counter[13]<<8)/100;                // Temperature
        // BP Value Systolic and Diastolic
        
        global_rr = (int) (CES_Pkt_Data_Counter[14]);
        ...
        else 
        {
          ...
          lblRR.setText("Respiration: " + global_rr+ " rpm");
          lblHR.setText("Heart Rate: " + global_hr + " bpm");          
        }
        ...
        if(updateCounter==100)
        {
          if (startPlot)
          {
            lblTemp.setText("Temperature: "+global_temp+" C");
        ...
        if (logging == true)
        {
          try 
          {
            ...
            bufferedWriter.write(ecg+","+spo2_red+","+resp+","+global_temp+","+global_hr+","+global_spo2+","+global_rr);
            bufferedWriter.newLine();
          }
          catch(IOException e) 
          {
